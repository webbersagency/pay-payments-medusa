...
4. Direct Debit (`PaymentProviderKeys.DIRECTDEBIT`) is special: payment collection is set to `AWAITING` (other methods go to `NOT_PAID`) and the session is marked `captured` synchronously. It is not created through the order API but through the direct debit API (`PayDirectDebitService.updatePayment`): by default the v3 endpoint (`rest-api.pay.nl/v3/DirectDebit/debitAdd/json`) is used; setting `directDebitApiVersion: "v2"` in the provider options switches to the mandate API (`POST rest.pay.nl/v2/directdebits/mandates`), which reports refunds/chargebacks but requires v2 directdebits API access on the account (403 otherwise). Both responses are normalized to the same session shape (`code` = mandate id), so downstream logic is version-agnostic and switching needs no data migration; the mandate response (its `code` is the mandate id) becomes the session data. Canceling such a payment deletes the mandate at Pay. (`DELETE /directdebits/mandates/{code}`, `PayDirectDebitService.cancelPayment`) so a pending collection is never executed; payments holding a Pay. order id still cancel through the order API abort. Collection status changes arrive later via incasso exchanges (see Webhooks).
