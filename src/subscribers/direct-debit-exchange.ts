...
const getPayClient = (
  ...
  return new PayClient(payModuleConfig.options as ProviderOptions, logger)
}
...
export default async function payDirectDebitExchangeHandler({
  ...
  const client = getPayClient(container, logger)
  ...
  if (isLegacyDirectDebitAction(action)) {
    ...
    if (!extractDirectDebit(info) && mandateId) {
  ...
  if (
    ...
    // A storno legitimately reverses a collected payment, so the collection is
    // marked failed even when a captured payment exists
